...
import { OrderBookControls } from "./OrderBookCotrols";

const parseSection: (section: [string, string][]) => [string, number, string][] = (section) => section.map(([price, qty]) => [Number(price).toFixed(2), Number(qty), (Number(price) * Number(qty)).toFixed(6)]);
  ...
  return (  
    <div>
      ...
      <div style={{ display: isConnected ? "block" : "none" }}>
            <div style={{ display: parsedOrderBook ? "flex" : "none", flexDirection: "column", alignItems: "stretch", gap: "20px", width: 320 }}>
                <OrderBookControls />
